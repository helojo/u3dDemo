        ...
        if (((this.mPressed && (this.mTouchID == UICamera.currentTouchID)) && (base.enabled && NGUITools.GetActive(base.gameObject))) && (this.target != null))
        {
            ...
            if (this.mPlane.Raycast(ray, out enter))
            {
                ...
                if (this.restrictWithinPanel)
                {
                    ...
                    if ((this.dragEffect != DragEffect.MomentumAndSpring) && this.mPanel.ConstrainTargetToBounds(this.target, ref this.mBounds, true))
                    {
                        this.CancelMovement();
                    }
                }
        ...
        if ((base.enabled && NGUITools.GetActive(base.gameObject)) && (this.target != null))
        {
            if (pressed)
            {
                if (!this.mPressed)
                {
                    ...
                    this.mPlane = new Plane((Vector3) (((this.mPanel == null) ? transform.rotation : this.mPanel.cachedTransform.rotation) * Vector3.back), UICamera.lastWorldPosition);
                }
            }
            else if (this.mPressed && (this.mTouchID == UICamera.currentTouchID))
            {
                this.mPressed = false;
                if ((this.restrictWithinPanel && (this.dragEffect == DragEffect.MomentumAndSpring)) && this.mPanel.ConstrainTargetToBounds(this.target, ref this.mBounds, false))
                {
                    this.CancelMovement();
                }
            }
