...
public class EventDelegate
{
    ...
    private void Cache()
    {
        ...
        if ((((this.mCachedCallback != null) && ((this.mCachedCallback.Target as MonoBehaviour) == this.mTarget)) && !(GetMethodName(this.mCachedCallback) != this.mMethodName)) || ((this.mTarget == null) || string.IsNullOrEmpty(this.mMethodName)))
        {
            return;
        }
        ...
        {
            Debug.LogError(string.Concat(new object[] { "Failed to bind ", baseType, ".", this.mMethodName, "\n", exception.Message }));
            return;
        }
    Label_011B:
        if (this.mMethod == null)
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    this.mParameters = new Parameter[parameters.Length];
                    int num = 0;
                ...
                while (index < length)
                {
                    this.mParameters[index].expectedType = parameters[index].ParameterType;
                    index++;
                }
            }
        }
    ...
    public bool Execute()
    {
        ...
        else
        {
            ...
            catch (ArgumentException exception)
            {
                ...
                {
                    str = str + parameters[0];
                    for (int i = 1; i < parameters.Length; i++)
                    {
                        str = str + ", " + parameters[i].ParameterType;
                    }
                }
                str = str + "\n  Received: ";
