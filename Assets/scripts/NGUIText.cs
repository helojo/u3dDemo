...
public static class NGUIText
{
    ...
    private static float[] mBoldOffset = new float[] { -0.5f, 0f, 0.5f, 0f, 0f, -0.5f, 0f, 0.5f };
            ...
            case Alignment.Justified:
                if (printedWidth >= (rectWidth * 0.65f))
                {
                    ...
                    for (int k = 1; index < verts.size; k++)
                    {
                        ...
                        float num15 = f - x;
                        float from = x * num10;
                        float num17 = from + num15;
                        float to = f * num10;
                        float num19 = to - num15;
                        float t = k * num9;
                        x = Mathf.Lerp(from, num19, t);
                        f = Mathf.Lerp(num17, to, t);
                        x = Mathf.Round(x);
                        f = Mathf.Round(f);
    ...
    public static int CalculateOffsetToFit(string text)
    {
        ...
        float rectWidth = NGUIText.rectWidth;
        int size = mSizes.size;
        while ((size > 0) && (rectWidth > 0f))
        {
            rectWidth -= mSizes[--size];
        }
        mSizes.Clear();
        if (rectWidth < 0f)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                else if (ch >= 0x20)
                {
                    ...
                    else
                    {
                        ...
                        if (Mathf.RoundToInt(num + num9) > rectWidth)
                        {
                            if (num > num3)
                            {
                                num3 = num - finalSpacingX;
    ...
    {
        return NGUIMath.DecimalToHex8(Mathf.Clamp(Mathf.RoundToInt(a * 255f), 0, 0xff));
    }
    ...
    public static GlyphInfo GetGlyph(int ch, int prev)
    {
        if (bitmapFont != null)
        {
            ...
            if (glyph != null)
            {
                ...
                NGUIText.glyph.u1.x = glyph.x + glyph.width;
                NGUIText.glyph.u1.y = glyph.y;
                NGUIText.glyph.advance = glyph.advance + num;
        ...
        {
            NGUIText.glyph.v0.x = mTempChar.vert.xMin;
            NGUIText.glyph.v1.x = NGUIText.glyph.v0.x + mTempChar.vert.width;
            NGUIText.glyph.v0.y = mTempChar.vert.yMax - baseline;
            NGUIText.glyph.v1.y = NGUIText.glyph.v0.y - mTempChar.vert.height;
            NGUIText.glyph.u0.x = mTempChar.uv.xMin;
            NGUIText.glyph.u0.y = mTempChar.uv.yMin;
            NGUIText.glyph.u1.x = mTempChar.uv.xMax;
            NGUIText.glyph.u1.y = mTempChar.uv.yMax;
            NGUIText.glyph.advance = mTempChar.width;
            NGUIText.glyph.channel = 0;
            NGUIText.glyph.rotatedUVs = mTempChar.flipped;
            ...
            if (num2 != 1f)
            {
                NGUIText.glyph.v0 = (Vector2) (NGUIText.glyph.v0 * num2);
                NGUIText.glyph.v1 = (Vector2) (NGUIText.glyph.v1 * num2);
    ...
    public static float GetGlyphWidth(int ch, int prev)
    {
        if (bitmapFont != null)
        {
            ...
            if (glyph != null)
            {
                return (fontScale * ((prev == 0) ? ((float) glyph.advance) : ((float) (glyph.advance + glyph.GetKerning(prev)))));
            }
        }
        else if ((dynamicFont != null) && dynamicFont.GetCharacterInfo((char) ch, out mTempChar, finalSize, fontStyle))
        {
    ...
    {
        return (((ch == 0x20) || (ch == 0x200a)) || (ch == 0x200b));
    }
    ...
    public static bool ParseSymbol(string text, ref int index, BetterList<Color> colors, bool premultiply, ref int sub, ref bool bold, ref bool italic, ref bool underline, ref bool strike)
    {
        ...
        if (text[index + 2] == ']')
        {
            ...
            if (str5 != null)
            {
                ...
                if (<>f__switch$map0.TryGetValue(str5, out num4))
                {
                    switch (num4)
                    {
                        case 0:
        ...
        if (text[index + 3] == ']')
        {
            ...
            if (str5 != null)
            {
                ...
                if (<>f__switch$map1.TryGetValue(str5, out num4))
                {
                    switch (num4)
                    {
                        case 0:
        ...
        if (text[index + 4] == ']')
        {
            ...
            if (str5 != null)
            {
                ...
                if (<>f__switch$map2.TryGetValue(str5, out num4))
                {
                    if (num4 == 0)
                    {
                        sub = 1;
        ...
        if (text[index + 5] == ']')
        {
            ...
            if (str5 != null)
            {
                ...
                if (<>f__switch$map3.TryGetValue(str5, out num4))
                {
                    switch (num4)
                    {
                        case 0:
    ...
    public static void Print(string text, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
    {
        if (!string.IsNullOrEmpty(text))
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                {
                    Color color4 = mColors[mColors.size - 1];
                    color4.a *= mAlpha;
                    tint = color4;
                    int num20 = 0;
                ...
                if (glyph != null)
                {
                    ...
                    if (Mathf.RoundToInt(x + advance) > rectWidth)
                    {
                        if (x == 0f)
                        {
                            return;
                    ...
                    if (!IsSpace(ch))
                    {
                        if (uvs != null)
                        {
                            if (bitmapFont != null)
                            {
                                ...
                                glyph.u1.x = uvRect.xMin + (num9 * glyph.u1.x);
                                glyph.u0.y = uvRect.yMax - (num10 * glyph.u0.y);
                                glyph.u1.y = uvRect.yMax - (num10 * glyph.u1.y);
                            }
                            int num31 = 0;
                            int num32 = !bold ? 1 : 4;
                            while (num31 < num32)
                        ...
                        if (cols != null)
                        {
                            if ((glyph.channel == 0) || (glyph.channel == 15))
                            {
                                if (gradient)
                                {
                                    float t = num11 + (y / fontScale);
                                    float num34 = num11 + (num29 / fontScale);
                                    t /= num11;
                                    num34 /= num11;
                                    s_c0 = Color.Lerp(a, b, t);
                                    s_c1 = Color.Lerp(a, b, num34);
                                    int num35 = 0;
                                    int num36 = !bold ? 1 : 4;
                                    while (num35 < num36)
                                    {
                        ...
                        if (!bold)
                        {
                            ...
                            {
                                float num41 = (fontSize * 0.1f) * ((num16 - num17) / ((float) fontSize));
                                verts.Add(new Vector3(num14 - num41, num17));
                                verts.Add(new Vector3(num14 + num41, num16));
                                verts.Add(new Vector3(num15 + num41, num16));
                                verts.Add(new Vector3(num15 - num41, num17));
                            }
                        }
                        else
                        {
                            for (int m = 0; m < 4; m++)
                            {
                                float num43 = mBoldOffset[m * 2];
                                float num44 = mBoldOffset[(m * 2) + 1];
                                float num45 = num43 + (!italic ? 0f : ((fontSize * 0.1f) * ((num16 - num17) / ((float) fontSize))));
                                verts.Add(new Vector3(num14 - num45, num17 + num44));
                                verts.Add(new Vector3(num14 + num45, num16 + num44));
                                verts.Add(new Vector3(num15 + num45, num16 + num44));
                                verts.Add(new Vector3(num15 - num45, num17 + num44));
                            }
                        }
                        if (underline || strike)
                        {
                            ...
                            if (info2 != null)
                            {
                                if (uvs != null)
                                {
                                    if (bitmapFont != null)
                                    {
                                        ...
                                        info2.u1.x = uvRect.xMin + (num9 * info2.u1.x);
                                        info2.u0.y = uvRect.yMax - (num10 * info2.u0.y);
                                        info2.u1.y = uvRect.yMax - (num10 * info2.u1.y);
                                    }
                                    float num46 = (info2.u0.x + info2.u1.x) * 0.5f;
                                    float num47 = (info2.u0.y + info2.u1.y) * 0.5f;
                                    uvs.Add(new Vector2(num46, num47));
                                    uvs.Add(new Vector2(num46, num47));
                                    uvs.Add(new Vector2(num46, num47));
                                    uvs.Add(new Vector2(num46, num47));
                                }
        ...
        while (index < length)
        {
            ...
            else
            {
                ...
                if (num12 != 0f)
                {
                    ...
                    float y = -num3 - num5;
                    float num16 = -num3;
                    if (Mathf.RoundToInt(num14 + finalSpacingX) > rectWidth)
                    {
                        if (x == 0f)
                        {
                            return;
        ...
        for (int i = 0; i < length; i++)
        {
            ...
            else
            {
                ...
                if (symbol == null)
                {
                    ...
                    {
                        glyphWidth += finalSpacingX;
                        if (Mathf.RoundToInt(x + glyphWidth) > rectWidth)
                        {
                            if (x == 0f)
                            {
                                return;
                ...
                else
                {
                    ...
                    if (Mathf.RoundToInt(x + num11) > rectWidth)
                    {
                        if (x == 0f)
                        {
                            return;
    ...
    public static void Update(bool request)
    {
        ...
        if ((dynamicFont != null) && request)
        {
            ...
            baseline = Mathf.Round(yMax + (((finalSize - yMax) + yMin) * 0.5f));
        }
    }
    ...
    {
        text = text.Replace(@"\n", "\n");
        if (((NGUIText.rectWidth < 1) || (rectHeight < 1)) || (finalLineHeight < 1f))
        {
            finalText = string.Empty;
            return false;
        }
        float num = (maxLines <= 0) ? ((float) rectHeight) : Mathf.Min((float) rectHeight, finalLineHeight * maxLines);
        ...
        while (index < length)
        {
            ...
            if ((IsSpace(ch) && !flag3) && (startIndex < index))
            {
                ...
                if (((num7 == b) && (rectWidth <= 0f)) && (index < length))
                {
                    char ch2 = text[index];
                    if ((ch2 < ' ') || IsSpace(ch2))
                    {
            ...
            if (Mathf.RoundToInt(rectWidth) < 0)
            {
                if (flag || (num7 == b))
                {
                    ...
                    {
                        startIndex = index;
                        rectWidth = NGUIText.rectWidth - num9;
                    }
                    prev = 0;
                    goto Label_03AC;
                }
