...
public class UIInput : MonoBehaviour
{
    ...
    protected int GetCharUnderMouse()
    {
        ...
        return (!plane.Raycast(currentRay, out num) ? 0 : (mDrawStart + this.label.GetCharacterIndexAtPosition(currentRay.GetPoint(num))));
    }

    protected string GetLeftText()
    {
        int a = Mathf.Min(this.mSelectionStart, this.mSelectionEnd);
        return ((!string.IsNullOrEmpty(this.mValue) && (a >= 0)) ? this.mValue.Substring(0, Mathf.Min(a, this.mValue.Length)) : string.Empty);
    }

    protected string GetRightText()
    {
        int a = Mathf.Max(this.mSelectionStart, this.mSelectionEnd);
        return ((!string.IsNullOrEmpty(this.mValue) && (a < this.mValue.Length)) ? this.mValue.Substring(Mathf.Min(a, this.mValue.Length)) : string.Empty);
    }
    ...
    private void OnGUI()
    {
        if (this.isSelected && (Event.current.rawType == UnityEngine.EventType.KeyDown))
        {
    ...
    protected virtual bool ProcessEvent(Event ev)
    {
        ...
        bool flag2 = (((platform != RuntimePlatform.OSXEditor) && (platform != RuntimePlatform.OSXPlayer)) && (platform != RuntimePlatform.OSXWebPlayer)) ? ((ev.modifiers & EventModifiers.Control) != 0) : ((ev.modifiers & EventModifiers.Command) != 0);
        bool flag3 = (ev.modifiers & EventModifiers.Shift) != 0;
    ...
    Label_04B7:
        ev.Use();
        if ((this.onReturnKey == OnReturnKey.NewLine) || ((((this.onReturnKey == OnReturnKey.Default) && this.label.multiLine) && (!flag2 && (this.label.overflowMethod != UILabel.Overflow.ClampContent))) && (this.validation == Validation.None)))
        {
            this.Insert("\n");
        }
        else
        ...
        if (this.isSelected)
        {
            ...
            if ((this.mSelectMe != -1) && (this.mSelectMe != Time.frameCount))
            {
                ...
                if (((Application.platform == RuntimePlatform.IPhonePlayer) || (Application.platform == RuntimePlatform.Android)) || ((Application.platform == RuntimePlatform.WP8Player) || (Application.platform == RuntimePlatform.BB10Player)))
                {
                    ...
                    mKeyboard = (this.inputType != InputType.Password) ? TouchScreenKeyboard.Open(mValue, keyboardType, this.inputType == InputType.AutoCorrect, this.label.multiLine && !this.hideInput, false, false, this.defaultText) : TouchScreenKeyboard.Open(mValue, keyboardType, false, false, true);
                }
                else
                {
    ...
    public bool inputShouldBeHidden
    {
        get
        {
            return ((this.hideInput && (this.label != null)) && !this.label.multiLine);
        }
    }
