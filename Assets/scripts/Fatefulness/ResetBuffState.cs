namespace Fatefulness
{
    ...
    public class ResetBuffState : Concentrator
    {
        public override void Concreate()
        {
            ...
            base.RegisterPort<Transition>("State", Transition.Direction.input);
        }
        ...
        public override void Excude(Intent intent)
        {
            ...
            if (actor != null)
            {
                port = base.GetPort("State");
                if (port == null)
                {
                    throw new UnityException("unexisted transition port named Entry");
                }
                port.Review(intent2);
                int state = intent2.GetInt32(IntentDecl.variable);
                actor.ResetBuffStateTime(state);
            }
        }
