...
public class UIDrawCall : MonoBehaviour
{
    ...
    private void CreateMaterial()
    {
        ...
        if (this.mClipCount != 0)
        {
            ...
            if (this.shader == null)
            {
                Shader.Find(name + " " + this.mClipCount);
            }
        ...
        if (this.mMaterial != null)
        {
            ...
            this.mDynamicMat.hideFlags = HideFlags.NotEditable | HideFlags.DontSave;
    ...
    private void OnWillRenderObject()
    {
        ...
        if ((this.mDynamicMat != null) && (this.mClipCount != 0))
        {
                ...
                while (parentPanel != null)
                {
                    if (parentPanel.hasClipping)
                    {
                        ...
                        if (parentPanel != this.panel)
                        {
                            ...
                            drawCallClipRange.x -= vector2.x;
                            drawCallClipRange.y -= vector2.y;
                            Vector3 eulerAngles = this.panel.cachedTransform.rotation.eulerAngles;
                            Vector3 vector5 = parentPanel.cachedTransform.rotation.eulerAngles - eulerAngles;
                            vector5.x = NGUIMath.WrapAngle(vector5.x);
                            vector5.y = NGUIMath.WrapAngle(vector5.y);
                            vector5.z = NGUIMath.WrapAngle(vector5.z);
                            if ((Mathf.Abs(vector5.x) > 0.001f) || (Mathf.Abs(vector5.y) > 0.001f))
                            {
            ...
            else
            {
                ...
                Vector4 vector7 = this.panel.drawCallClipRange;
                Vector2 vector8 = new Vector2(-vector7.x / vector7.z, -vector7.y / vector7.w);
                Vector2 vector9 = new Vector2(1f / vector7.z, 1f / vector7.w);
                Vector2 vector = new Vector2(1000f, 1000f);
                if (clipSoftness.x > 0f)
                {
                    vector.x = vector7.z / clipSoftness.x;
                }
                if (clipSoftness.y > 0f)
                {
                    vector.y = vector7.w / clipSoftness.y;
                }
                this.mDynamicMat.mainTextureOffset = vector8;
                this.mDynamicMat.mainTextureScale = vector9;
