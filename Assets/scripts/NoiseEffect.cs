...
[ExecuteInEditMode, AddComponentMenu("Image Effects/Noise/Noise and Scratches"), RequireComponent(typeof(Camera))]
public class NoiseEffect : MonoBehaviour
{
    ...
    private void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        ...
        mat.SetVector("_GrainOffsetScale", new Vector4(UnityEngine.Random.value, UnityEngine.Random.value, (((float) Screen.width) / ((float) this.grainTexture.width)) * num, (((float) Screen.height) / ((float) this.grainTexture.height)) * num));
        mat.SetVector("_ScratchOffsetScale", new Vector4(this.scratchX + (UnityEngine.Random.value * this.scratchJitter), this.scratchY + (UnityEngine.Random.value * this.scratchJitter), ((float) Screen.width) / ((float) this.scratchTexture.width), ((float) Screen.height) / ((float) this.scratchTexture.height)));
        mat.SetVector("_Intensity", new Vector4(UnityEngine.Random.Range(this.grainIntensityMin, this.grainIntensityMax), UnityEngine.Random.Range(this.scratchIntensityMin, this.scratchIntensityMax), 0f, 0f));
